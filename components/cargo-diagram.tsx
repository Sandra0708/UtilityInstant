...
export default function CargoDiagram({space,items,placed,view,en,onSelect}:{space:Space;items:Cargo[];placed:Placed[];view:'top'|'side'|'perspective';en:boolean;onSelect?:(p:Placed)=>void}){
 ...
 return <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 910 500" role="img" aria-label={en?'Cargo layout with internal dimensions':'Distribución de carga con medidas interiores'} style={{color:'#7f8fa8'}}>
 <title>{en?'Load plan':'Plano de carga'} · {L} × {W} × {H} mm</title>
 ...
 {(space.zones||[]).map(zone=>{const {x,y,width:w,length:l}=zone;const corners=side?[point(x,y),point(x,y+l),point(x,y+l,H),point(x,y,H)]:[point(x,y),point(x,y+l),point(x+w,y+l),point(x+w,y)];return <polygon key={zone.id} points={poly(corners)} fill={zone.kind==='aisle'?'#13a18a':'#d04b59'} fillOpacity=".22" stroke={zone.kind==='aisle'?'#13a18a':'#d04b59'} strokeDasharray="4 3"><title>{zone.kind==='aisle'?(en?'Reserved aisle':'Pasillo reservado'):(en?'Blocked zone':'Zona bloqueada')}</title></polygon>;})}
 ...
 <text x="45" y="25" fill="currentColor" fontSize="16" fontFamily="Arial">{en?'Length':'Fondo'}: {L} mm · {en?'Width':'Ancho'}: {W} mm · {en?'Height':'Alto'}: {H} mm</text>
 <text x="45" y="450" fill="currentColor" fontSize="13" fontFamily="Arial">{side?(en?'Side projection: nearer pallets may hide others':'Proyección lateral: los palets próximos pueden ocultar otros'):(en?'Red: blocked · Green: reserved aisle':'Rojo: zona bloqueada · Verde: pasillo reservado')}</text>
 <text x="45" y="475" fill="currentColor" fontSize="14" fontFamily="Arial">{en?'Door / access: left · Click a pallet for its position':'Puerta / acceso: izquierda · Pulsa un palet para ver su posición'}</text>
 </svg>;
}
