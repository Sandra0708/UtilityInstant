...
export default function CargoOrbit({space,items,placed,en,onSelect}:{space:Space;items:Cargo[];placed:Placed[];en:boolean;onSelect:(p:Placed)=>void}){
 ...
 return <><div style={{display:'flex',gap:8,flexWrap:'wrap',padding:10}}><span>{en?'Drag to rotate · Arrow keys to turn':'Arrastra para girar · Flechas del teclado para rotar'}</span><button type="button" onClick={()=>setAngle(a=>({...a,zoom:Math.min(3,a.zoom*1.2)}))} aria-label={en?'Zoom in':'Acercar'}>+</button><button type="button" onClick={()=>setAngle(a=>({...a,zoom:Math.max(.5,a.zoom/1.2)}))} aria-label={en?'Zoom out':'Alejar'}>−</button><button type="button" onClick={()=>setAngle({yaw:.65,pitch:.55,zoom:1})}>{en?'Reset view':'Restablecer vista'}</button></div>
 <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 910 500" tabIndex={0} role="img" aria-label={en?'Interactive 3D load: drag or use arrow keys':'Carga 3D interactiva: arrastra o usa las flechas'} style={{touchAction:'none',cursor:'grab',width:'100%'}} onPointerDown={e=>{drag.current={x:e.clientX,y:e.clientY,id:e.pointerId};e.currentTarget.setPointerCapture(e.pointerId);}} onPointerMove={e=>{const d=drag.current;if(!d||d.id!==e.pointerId)return;const dx=e.clientX-d.x,dy=e.clientY-d.y;drag.current={x:e.clientX,y:e.clientY,id:e.pointerId};setAngle(a=>({...a,yaw:a.yaw+dx*.01,pitch:Math.max(-1.5,Math.min(1.5,a.pitch+dy*.01))}));}} onPointerUp={()=>{drag.current=null;}} onPointerCancel={()=>{drag.current=null;}} onKeyDown={e=>{if(!['ArrowLeft','ArrowRight','ArrowUp','ArrowDown'].includes(e.key))return;e.preventDefault();setAngle(a=>({...a,yaw:a.yaw+(e.key==='ArrowLeft'?-.15:e.key==='ArrowRight'?.15:0),pitch:Math.max(-1.5,Math.min(1.5,a.pitch+(e.key==='ArrowUp'?.15:e.key==='ArrowDown'?-.15:0)))}));}}>
