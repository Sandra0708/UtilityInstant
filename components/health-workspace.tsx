...
export default function HealthWorkspace({id,locale,prefill}:{id:HealthId;locale:Language;prefill?:Record<string,string>}) {
 ...
 const option=(v:string)=>v==='mifflin'?t('unknownFatMethod'):v==='katch'?t('knownFatMethod'):v==='navy'?t('measuredMethod'):v==='deurenberg'?t('unmeasuredMethod'):activityKeys[['1.2','1.375','1.55','1.725','1.9'].indexOf(v)]?t(activityKeys[['1.2','1.375','1.55','1.725','1.9'].indexOf(v)]):t(v as HealthKey);
 ...
 function select(key:string,label:HealthKey,options:string[]) {const help:HealthKey=key==='region'?'regionHelp':key==='sex'?'sexHelp':key==='activity'?'activityHelp':key==='method'&&id==='tdee'?'calorieMethodHelp':key==='method'&&id==='body-fat'?'fatMethodHelp':helpKeys[id];return <HelpLabel locale={locale} optional={false} help={t(help)} className={s.field} key={key}><span>{t(label)}</span><select value={values[key]} onChange={e=>change(key,e.target.value)}>{options.map(v=><option key={v} value={v}>{option(v)}</option>)}</select></HelpLabel>;}
