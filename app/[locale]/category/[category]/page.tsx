...
export async function generateMetadata({params}:{params:Promise<{locale:Locale;category:string}>}){const {locale,category}=await params;const cat=categories.find(c=>c.id===category);if(!cat)return {};return {title:cat.title[locale]+' · UtilityInstant',description:locale==='es'?`Herramientas gratuitas de ${cat.title.es.toLowerCase()} con procesamiento local.`:`Free ${cat.title.en.toLowerCase()} tools with local processing.`,alternates:alternate(locale,'/category/'+category)};}
