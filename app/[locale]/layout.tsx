...
export default async function LocaleLayout({children,params}:{children:React.ReactNode;params:Promise<{locale:string}>}){const {locale}=await params;if(locale!=='es'&&locale!=='en')notFound();return <Provider locale={locale as Locale}><a className="skip-link" href="#main">{locale==='es'?'Saltar al contenido':'Skip to content'}</a>{children}</Provider>;}
