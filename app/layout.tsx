...
export default async function RootLayout({children,params}: {children: React.ReactNode;params:Promise<{locale?:string}>}){const locale=(await headers()).get('x-nexo-locale')||'es';return <html lang={locale==='en'?'en':'es'} suppressHydrationWarning><head><script dangerouslySetInnerHTML={{__html:"try{if(JSON.parse(localStorage.getItem('nexo.preferences')||'{}').theme==='dark')document.documentElement.classList.add('dark')}catch(e){}"}}/></head><body><AdsBootstrap/>{children}</body></html>;}
