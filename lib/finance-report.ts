...
export async function financePdf(report:FinanceReport,format:(v:number|null,unit?:string)=>string,svg:SVGSVGElement|null,inputs:[string,string][]){
 ...
 page('Resumen / Summary');report.metrics.forEach((m,i)=>{ctx.fillStyle='#183354';ctx.font='22px Arial';ctx.fillText(m.label+': '+format(m.value,m.unit),50+(i%2)*800,165+Math.floor(i/2)*38,760);});
 ...
 table(report.columns,report.rows,'Tabla completa / Full schedule');table(['Dato / Input','Valor / Value'],inputs,'Supuestos / Assumptions');return imagePagesPdf(images);
}
